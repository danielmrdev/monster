  ...
  register(): Worker {
    ...
    const worker = new Worker<SslPollerPayload>(
      ...
      async (job) => {
        ...
        console.log(
          `[SslPollerJob] attempt ${attempt}/${MAX_ATTEMPTS} for site ${siteId} (zone ${cfZoneId})`,
        );
        ...
        if (sslStatus === "active") {
          ...
          if (
            SITE_STATUS_FLOW[currentStatus as keyof typeof SITE_STATUS_FLOW]?.includes(
              "ssl_pending",
            )
          ) {
        ...
        console.log(
          `[SslPollerJob] re-enqueued attempt ${attempt + 1} for site ${siteId} (60s delay)`,
        );
      },
      { connection },
    );

  ...
  register(): Worker {
    ...
    const worker = new Worker<GenerateSitePayload>(
      ...
      async (job) => {
        ...
        console.log(
          `[GenerateSiteJob] Starting job ${job.id} for site ${siteId}`,
        )
        ...
        if (siteError || !site) {
          throw new Error(
            `Site not found: ${siteId} — ${siteError?.message ?? "null row"}`,
          )
        }
        ...
        const autoDeployAfterGenerate =
          previousStatus === "live" || previousStatus === "paused"
        ...
        if (existingJob) {
          ...
          if (insertErr) {
            console.error(
              `[GenerateSiteJob] Failed to insert ai_jobs: ${insertErr.message}`,
            )
        ...
        for (const cat of dbCategoriesRaw) {
          ...
          const links = (catProductLinks ?? []).filter(
            (cp) => cp.category_id === cat.id,
          )
        ...
        const imagesDownloaded = [...imageMap.values()].filter(
          (p) => p.length > 0,
        ).length
        console.log(
        ...
        const siteData = {
          site: {
            ...
            domain: (site.domain ??
              `${slugify(site.name as string)}.example.com`) as string,
            ...
            homepage_intro: ((site as Record<string, unknown>).homepage_intro ??
              null) as string | null,
          ...
          products: dbProducts
            ...
            .map((p) => ({
              ...
              pros_cons:
                p.pros_cons as { pros: string[] cons: string[] } | null ?? null,
        ...
        try {
          ...
          if (assignments && assignments.length > 0) {
            ...
            const { data: templates, error: tplErr } =
              await (supabase as unknown as {
                from: (
                  table: string,
                ) => {
            ...
            const templateMap = new Map(
              (templates ?? []).map((t) => [t.id, t.content]),
            )

            for (const a of assignments) {
              const type =
                a.template_type as "privacy" | "terms" | "cookies" | "contact"
          ...
          console.warn(
            "[GenerateSiteJob] legal templates fetch failed (non-fatal):",
            e,
          )
        }
        ...
        if (logoSrc) {
          ...
          if (existsSync(srcPath)) {
            ...
            console.warn(
              `[GenerateSiteJob] logo source not found: ${srcPath} — skipping`,
            )
          }
        }
        ...
        if (faviconSrc) {
          ...
          if (existsSync(srcDir)) {
            ...
            console.warn(
              `[GenerateSiteJob] favicon source dir not found: ${srcDir} — skipping`,
            )
          }
        }
        ...
        for (const relPath of htmlFiles) {
          ...
          if (relPath.startsWith("go/") || inferPageType(relPath) === "legal")
            continue
          try {
            ...
            console.error(
              `[GenerateSiteJob] score_pages: error scoring ${relPath}:`,
              err,
            )
          }
        }
        ...
        if (scoreRows.length > 0) {
          ...
          if (upsertError) {
            console.error(
              "[GenerateSiteJob] score_pages: upsert error:",
              upsertError.message,
            )
          } else {
            console.log(
        ...
        try {
          ...
          console.warn(
            `[GenerateSiteJob] seo_files: non-fatal error writing SEO files:`,
            seoErr,
          )
        }
        ...
        if (autoDeployAfterGenerate) {
          ...
          await supabase
            ...
            .eq("id", siteId)
          console.log(
            `[GenerateSiteJob] site ${siteId}: → generated (awaiting manual deploy)`,
          )
        }

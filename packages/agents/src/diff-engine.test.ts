...
describe("price changes", () => {
  it("triggers rebuild when price changes beyond epsilon", () => {
    const result = diffProducts([makeDb({ current_price: 9.99 })], [makeDfs({ price: 12.99 })]);
  ...
  it("does NOT trigger rebuild when price difference is within epsilon (< 0.01)", () => {
    const result = diffProducts([makeDb({ current_price: 9.99 })], [makeDfs({ price: 9.991 })]);
  ...
  it("triggers rebuild when price goes from null to a number", () => {
    const result = diffProducts([makeDb({ current_price: null })], [makeDfs({ price: 19.99 })]);
    const priceChanges = result.changes.filter((c) => c.type === "price");
...
describe("rating changes", () => {
  it("does NOT trigger rebuild when only rating changes", () => {
    const result = diffProducts([makeDb({ rating: 4.5 })], [makeDfs({ rating: 3.8 })]);
    ...
    const result = diffProducts(
      [
        makeDb({
          rating: 4.5,
          current_price: 9.99,
          source_image_url: "https://m.media-amazon.com/images/same.jpg",
        }),
      ],
      [
        makeDfs({
          rating: 3.0,
          price: 9.99,
          imageUrl: "https://m.media-amazon.com/images/same.jpg",
        }),
      ],
    );
    expect(result.shouldRebuild).toBe(false);

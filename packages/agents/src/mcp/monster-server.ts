 ...
 */
export function createMonsterMcpServer(
    ...
    tools: [
        ...
        async (args, _extra) => {
          ...
          if (isUuid) {
            ({ data, error } = await supabase
              .from('sites')
              .select('id, name, domain, niche, status, market, language, template_slug, created_at, updated_at')
      ...
      tool(
        ...
        async (args, _extra) => {
          ...
          console.log(`[monster-mcp] tool=getAnalytics called site_id=${args.site_id} days=${days}`);

  ...
  return createSdkMcpServer({
    ...
    tools: [
      tool(
        ...
        async (args, _extra) => {
          console.log(`[niche-mcp] tool=keywordIdeas called keyword="${args.keyword}" market=${args.market}`);
          try {
      ...
      tool(
        ...
        {
          keywords: z.array(z.string()).min(1).max(10).describe('Array of keywords to analyze (1-10)'),
          market: z.string().describe('Amazon market code: ES, US, UK, DE, FR, IT'),
        },
        async (args, _extra) => {
          console.log(`[niche-mcp] tool=serpCompetitors called keywords=${args.keywords.length} market=${args.market}`);
          try {
      ...
      tool(
        ...
        async (args, _extra) => {
          console.log(`[niche-mcp] tool=googleSerpResults called keyword="${args.keyword}" market=${args.market}`);
          try {
      ...
      tool(
        ...
        async (args, _extra) => {
          console.log(`[niche-mcp] tool=amazonProducts called keyword="${args.keyword}" market=${args.market} (async task — may take 30-60s)`);
          try {
      ...
      tool(
        ...
        async (args, _extra) => {
          ...
          try {
            ...
            console.log(`[niche-mcp] tool=checkDomainAvailability result available=${result.available} domain="${args.domain}"`);

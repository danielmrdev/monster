...
function scoreMetaElements($: cheerio.CheerioAPI, keyword: string, pageType: PageType): number {
  const isLegal = pageType === "legal";
  let score = 0;
...
function buildSuggestions(scores: {
  ...
  if (scores.media < 50)
    suggestions.push("Add images with descriptive alt text containing the focus keyword.");
  if (scores.schema < 30)
    suggestions.push(
      "Add JSON-LD structured data matching the page type (e.g. Product, CollectionPage).",
    );
  if (scores.technical < 60)
    suggestions.push("Add a viewport meta tag and set the <html lang> attribute.");
  if (scores.social < 50)
    suggestions.push("Add Open Graph meta tags (og:title, og:type, og:image, og:url).");

              ...
              <CardContent>
                {isAlerts ? (
                  <div className="flex items-end justify-between">
                    <p className={`text-3xl font-bold${hasAlerts ? ' text-amber-500' : ''}`}>{value}</p>
                    {alertCount > 0 && (
            ...
            {totalRevenue === 0 && totalCosts === 0 ? (
              ...
              <div className="space-y-2">
                ...
                <div className="border-t border-border pt-2 flex justify-between text-sm font-semibold">
                  <span>Profit</span>
                  <span className={`font-mono ${profit > 0 ? 'text-green-400' : profit < 0 ? 'text-red-400' : 'text-muted-foreground'}`}>
                    {fmtCurrency(profit)}
                  </span>
                </div>
                <Link href="/finances" className="block text-xs text-muted-foreground hover:text-foreground transition-colors pt-1">
                  Full P&amp;L →
                </Link>
              </div>
            )}
              ...
              <div className="space-y-1.5">
                {topSites.map(({ site_id, total }) => (
                  <div key={site_id} className="flex justify-between items-center text-sm">
                    <Link href={`/sites/${site_id}`} className="text-foreground hover:text-primary transition-colors truncate max-w-[60%]">
                      {siteNameById.get(site_id) ?? 'Unknown'}
                    </Link>
                    <span className="font-mono text-muted-foreground text-xs">{total.toLocaleString()}</span>
                  </div>
                ))}
                <Link href="/analytics" className="block text-xs text-muted-foreground hover:text-foreground transition-colors pt-1">
                  Full analytics →
                </Link>
              </div>
            )}
                    ...
                    return (
                      <tr key={job.id}>
                        <td className="py-2 pr-4 text-foreground">
                          {job.site_id ? (
                            <Link href={`/sites/${job.site_id}`} className="hover:text-primary transition-colors">
                              {siteName}
                            </Link>
                          ) : '—'}
                        </td>
                        <td className="py-2 pr-4 text-muted-foreground font-mono text-xs">{job.job_type}</td>
                        <td className="py-2 pr-4 text-muted-foreground text-xs whitespace-nowrap">{fmt(job.started_at)}</td>
                        <td className="py-2 text-red-400 text-xs font-mono truncate max-w-[300px]" title={job.error ?? ''}>

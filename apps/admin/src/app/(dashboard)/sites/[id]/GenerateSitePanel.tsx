...
export function GenerateSitePanel({ siteId, domain }: GenerateSitePanelProps) {
  ...
  const payload = currentJob?.payload as { phase?: string; done?: number; total?: number } | null;
  ...
  const showStatus = phase === "running" || phase === "pending" || phase === "completed" || (phase === "failed" && error);

  return (
    <div className="rounded-xl border border-border bg-card px-6 py-5">
      {/* Header: title + button */}
      <div className={`flex items-center justify-between${showStatus ? " mb-4" : ""}`}>

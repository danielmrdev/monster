  ...
  if (compact) {
    return (
      ...
      >
        {badge.label}
        {status === "completed" && score != null && (
          <span className="ml-1 opacity-75" title="Content quality score of the raw markdown">
            · cq:{score}
          </span>
        )}
      </span>
    ...
    <div className="mt-3 rounded-md border border-border bg-muted/30 px-4 py-3 text-sm space-y-1">
      <div className="flex items-center gap-2">
        ...
        {status === "completed" && totalProcessed != null && (
          <span className="text-xs text-muted-foreground">{totalProcessed} processed</span>
        )}
      </div>

...
export default function SeoJobStatus({ siteId, jobType, entityId, compact }: Props) {
  ...
  if (!job) {
    ...
    return <p className="text-sm text-muted-foreground/70 mt-2">No SEO jobs yet.</p>;
  }
  ...
  const result = job.result as { score?: number; attempts?: number; totalProcessed?: number } | null;
  ...
  if (compact) {
    return (
      <span
        ...
        title={status === 'completed' && score != null ? `Score: ${score}, Attempts: ${attempts ?? 1}` : undefined}
      >
        {badge.label}

...
export async function createSite(
  ...
  const rawSite = {
    ...
    affiliate_tag: (formData.get("affiliate_tag") as string) || undefined,
  ...
  const { name, domain, niche, market, language, currency, affiliate_tag, template_slug } =
    siteResult.data;
  const customization = customizationResult.data;
  const customizationJson = Object.fromEntries(
    Object.entries(customization).filter(([, v]) => v !== undefined && v !== ""),
  );
...
export async function updateSite(
  ...
  const homepageMetaDescription = (formData.get("homepage_meta_description") as string) || null;

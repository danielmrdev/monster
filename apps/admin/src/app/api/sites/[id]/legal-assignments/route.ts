...
export async function POST(
  ...
  // Verify site exists
  const { data: site } = await (createServiceClient()).from('sites').select('id').eq('id', siteId).single()
  ...
  for (const type of legalTypes) {
    ...
    if (templateId) {
      ...
      if (error) {
        return NextResponse.json({ error: `Failed to assign ${type}: ${error.message}` }, { status: 500 })
      }
    } else {
